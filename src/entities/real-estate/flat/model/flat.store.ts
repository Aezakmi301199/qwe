...
import { Flat, FlatClusterType } from './type';
...
import { FlatFilters, FlatFilterType } from './use-flat-filters';
...
export class FlatStore implements Store<Flat> {
  @observable realEstate: Flat[] = [];
  @observable comments: Comment[] = [];
  @observable count: number = 0;
  @observable currentFilter: null | FlatFilterType = null;
  ...
  @action
  setCurrentFilter(objectFilter: FlatFilterType) {
    runInAction(() => {
  ...
  @action
  changeValueByCurrentFilter<T extends keyof FlatFilterType>(fieldName: T, value: FlatFilterType[T]) {
  ...
  @action
  removeFieldByFilter<T extends keyof FlatFilterType>(fieldName: T) {
    if (!this.currentFilter) {
      return;
    }
  ...
  removeById(realEstateId: string) {
    this.realEstate = this.realEstate.filter((flat) => flat.id !== realEstateId);
  }

  @action
  findById(realEstateId: string) {
    return this.realEstate.find((flat) => flat.id === realEstateId);
  }

  async fetchByIdFromApi(realEstateId: string) {
    try {
      const res = await axios.get<Flat>(`${environments.REACT_APP_PROXY}/api/flats/${realEstateId}`);
  ...
  async fetchDataClusters(zoom: number, coordinates: ExtremeCoordinatesLatLng) {
    ...
    try {
      ...
      const filteredObjectClusterFilter = spyObjectByFieldValue({
        ...
        exceptionTransformField: [FlatFilters.ADDRESS],
      });
      ...
      const dataFlats: ClusterByRealEstateDto[] = await axios
        .post(`${environments.REACT_APP_PROXY}/api/flats/clusters`, limitGeoParamsRealEstateClusters(clusterFilter), {})
        .then((response) => response.data);

      runInAction(() => {
        this.clusters = dataFlats;
      });
    } catch (error) {
  ...
  async fetchData(startTime?: DateTime, geometry?: GeoJsonGeometryType, zoom?: number) {
    ...
    const response = await axios
      .post(
        `${environments.REACT_APP_PROXY}/api/flats`,
        {
      ...
      .then((response: AxiosResponse<Flat[]>) => response)
      .catch((axiosError: AxiosError<AxiosErrorData>) => axiosError.response);

    match(response)
      .with({ status: HttpStatusCode.SUCCESS }, (response: AxiosResponse<Flat[]>) => {
        runInAction(() => {
  ...
  async fetchNewData() {
    ...
    const oldIds = new Set(this.realEstate.map((flat) => flat.id));
    const newFlats = response.data.filter((newFlat) => !oldIds.has(newFlat.id));
    const newFlatsWithNewProp = newFlats.map((flat) => {
      return { ...flat, isNew: true };
    });

    runInAction(() => {
      this.realEstate.forEach((flat) => {
        flat.isNew = false;
      });
    });

    if (newFlats.length) {
      this.realEstate = this.realEstate.filter((flat) => !flat.isNew);
      this.realEstate.unshift(...newFlatsWithNewProp);
    }
  }
  ...
  async summaryFetchData(page: number, zoom: number, geometry?: GeoJsonGeometryType) {
    ...
    try {
      ...
      const dataFlats: Flat[] = await axios
        .post(`${environments.REACT_APP_PROXY}/api/flats`, {
          page: page,
          limit: this.rowsPerPage,
          ...filteredObjectFilter,
          geometry,
      ...
      runInAction(() => {
        this.realEstate = [...this.realEstate, ...dataFlats];
      });

      return !!dataFlats.length;
    } catch (error) {
      handleAxiosError(error);
    }
  ...
  @action
  async fetchCount(objectFilter: FlatFilterType) {
    ...
    const response = await axios
      .post(`${environments.REACT_APP_PROXY}/api/flats/count`, {
        ...filteredObjectFilter,
      })
  ...
  async fetchComments(realEstateId: string, page: number, commentCount: number) {
    try {
      ...
      const comments: Comment[] = await axios
        .get(`${environments.REACT_APP_PROXY}/api/flats/${realEstateId}/comments?page=${page}`)
        .then((response) => {
  ...
  async addToFavourites(realEstateId: string) {
    try {
      await axios.post(`${environments.REACT_APP_PROXY}/api/flats/${realEstateId}/favourites`).then(() => {
        const flat = this.findById(realEstateId);
  ...
  async fetchFavourite(user: User, domain: string[], orderBy: TypeOrderBy) {
    ...
    try {
      const dataFlats: Flat[] = await axios
        .get(`${environments.REACT_APP_PROXY}/api/users/${user.id}/favourite-flats`, {
          params: {
            page: this.page,
            limit: this.rowsPerPage,
        ...
        .then((res) => {
          return res.data.map((flat: Record<string, any>) => {
            return { ...flat, isFavourite: Boolean(flat.usersFavourites.length) };
          });
        });

      runInAction(() => {
        this.realEstate = dataFlats;
        this.loading = false;
  ...
  async fetchFavoriteCount(userId: string) {
    try {
      axios.get(`${environments.REACT_APP_PROXY}/api/users/${userId}/favourite-flats/count`).then((response) => {
        runInAction(() => {
          this.count = response.data.count;
        });
      });
  ...
  cancelReservationForCall(realEstateId: string) {
    ...
    flat.callingUser = null;
  }

  @action
  handleDisconnect(userId: string) {
    this.realEstate.forEach((flat) => {
      if (flat.callingUser?.id === userId) {
        flat.callingUser = null;
      }
    });
  }
  ...
  makeCallEmit(realEstateId: string) {
    socket.emit(WsEvent.MAKE_CALL, { id: realEstateId, realEstateType: RealEstateTypeUpperCase.FLAT });
  }
  ...
  saveRealEstateEmit(realEstateId: string) {
    socket.emit(WsEvent.SAVE_REAL_ESTATE, { id: realEstateId, realEstateType: RealEstateTypeUpperCase.FLAT });
  }
  ...
  async removeFromFavourites(realEstateId: string) {
    try {
      await axios.delete(`${environments.REACT_APP_PROXY}/api/flats/${realEstateId}/favourites`).then(() => {
        const flat = this.findById(realEstateId);
