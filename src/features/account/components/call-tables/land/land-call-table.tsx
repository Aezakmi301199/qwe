...
const LandCallTable = observer(() => {
  const { landCallStore, landStore, catalogStore } = useRootStore();
  ...
  useEffect(() => {
    landCallStore.fetchCallsCount(user.id);
    landCallStore.fetchCalls(user.id);
  }, [landCallStore.page]);
  ...
  const getElementById = async (id: string) => {
    await landStore.fetchByIdFromApi(id);
  };
  ...
  return (
    <>
      ...
      >
        {landCallStore.realEstateCalls.length ? (
          <Table stickyHeader>
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              {landCallStore.realEstateCalls.map((call) => {
                const id = call.id;
                const isPlaying = playbackState[id];
                ...
                return (
                  <TableRow key={id}>
                    ...
                    <TableCell
                      onClick={async () => {
                        await getElementById(call.land.id);
                        setOpenedMakeCallDrawerId(call.land.id);
                        setOpenedCommentDrawerId(call.land.id);
                      }}
                    >
                    ...
                    <TableCell>
                      <Stack direction='row' alignItems='flex-start' gap='16px'>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          ...
                          >
                            {call.land.address ? call.land.address : 'Адрес не указан'}
                          </Typography>
                          <audio
                            ref={(ref) => {
                              if (ref) {
                      ...
                      <>
                        {call.recognitionStatus === RecognitionStatus.DONE && (
                          <Button
                            onClick={async () => {
                              await getElementById(call.land.id);
                              setOpenedRecognizedDrawerId(call.land.id);
                            }}
                            variant={'outlined'}
                            sx={{
                              borderRadius: '8px',
            ...
            {openedMakeCallDrawerId && landStore.realEstate && (
              <DrawerObjectInfo
                store={landStore}
            ...
            {openedRecognizedDrawerId && landStore.realEstate && (
              <DrawerRecognition
                ...
                state={{ right: true }}
                text={landCallStore.realEstateCalls
                  .find((call) => call.land.id === openedRecognizedDrawerId)
      ...
      <TablePagination
        component='div'
        count={landCallStore.realEstateCallsCount}
        page={landCallStore.page - 1}
        onPageChange={(_, newPage) => landCallStore.setPage(newPage + 1)}
