...
const FlatCallTable = observer(() => {
  const { flatCallStore, flatStore, catalogStore } = useRootStore();
  ...
  useEffect(() => {
    flatCallStore.fetchCalls(user.id);
    flatCallStore.fetchCallsCount(user.id);
  }, [flatCallStore.page]);
  ...
  const getElementById = async (id: string) => {
    await flatStore.fetchByIdFromApi(id);
  };
  ...
  return (
    <>
      ...
      >
        {flatCallStore.realEstateCalls.length ? (
          <Table stickyHeader>
            <TableHead>
              <TableRow>
            ...
            <TableBody>
              {flatCallStore.realEstateCalls.map((call) => {
                const id = call.id;
                const isPlaying = playbackState[id];
                ...
                return (
                  <TableRow key={id}>
                    ...
                    <TableCell
                      onClick={async () => {
                        await getElementById(call.flat.id);
                        setOpenedMakeCallDrawerId(call.flat.id);
                        setOpenedCommentDrawerId(call.flat.id);
                      }}
                    >
                    ...
                    <TableCell>
                      <Stack direction='row' alignItems='flex-start' gap='16px'>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          ...
                          >
                            {call.flat.address ? call.flat.address : 'Адрес не указан'}
                          </Typography>
                          <audio
                            ref={(ref) => {
                              if (ref) {
                      ...
                      <>
                        {call.recognitionStatus === RecognitionStatus.DONE && (
                          <Button
                            onClick={async () => {
                              await getElementById(call.flat.id);
                              setOpenedRecognizedDrawerId(call.flat.id);
                            }}
                            variant={'outlined'}
                            sx={{
                              borderRadius: '8px',
            ...
            {openedMakeCallDrawerId && flatStore.realEstate && (
              <DrawerObjectInfo
                store={flatStore}
            ...
            {openedRecognizedDrawerId && flatStore.realEstate && (
              <DrawerRecognition
                ...
                state={{ right: true }}
                text={flatCallStore.realEstateCalls
                  .find((call) => call.flat.id === openedRecognizedDrawerId)
      ...
      <TablePagination
        component='div'
        count={flatCallStore.realEstateCallsCount}
        page={flatCallStore.page - 1}
        onPageChange={(_, newPage) => flatCallStore.setPage(newPage + 1)}
