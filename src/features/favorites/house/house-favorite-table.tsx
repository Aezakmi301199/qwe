...
const HouseFavoriteTable: React.FC<FlatFavoriteTableProps> = observer(
  ({
    columnVisibility,
    ...
    }, [houseStore.page, houseStore.rowsPerPage]);
    ...
    socket
      ...
      .on(WsEvent.REAL_ESTATE_SAVED, async ({ realEstateId, userId }: { realEstateId: string; userId: string }) => {
        ...
        await houseStore.removeFromFavourites(realEstateId);
      });
    socket
                    ...
                    >
                      ...
                      <TableCell align='right' sx={{ verticalAlign: 'top', padding: '4px 16px 0 0px' }}>
                        <Stack flexDirection={'row'} alignItems={'flex-start'} justifyContent={'end'}>
                          ...
                          <Box
                            visibility={isPriceHistoryVisible(house.price, house.priceHistory) ? 'visible' : 'hidden'}
                          >
                        ...
                        {house.callingUser ? (
                          ...
                          >
                            <Button
                              ...
                              disabled={
                                Boolean(house.callingUser) || hasFullDuplicate(house.duplicates) || !house.isActive
                              }
                              variant={'contained'}
                              color={hasFullDuplicate(house.duplicates) || !house.isActive ? 'secondary' : 'primary'}
                              size={Size.SMALL}
                              sx={{
                                padding: '4px 10px',
                                boxShadow: 'none',
                                ...
                                fontWeight: '400',
                                color:
                                  hasFullDuplicate(house.duplicates) || !house.isActive
                                    ? `${theme.palette.text.primary} !important`
                                    : 'white',

                                borderRadius: `${
                                  house.isActive && !hasFullDuplicate(house.duplicates) ? '8px' : '45px'
                                }`,
                                textTransform: 'none',
                                whiteSpace: 'nowrap',
                                '&:hover': {
                            ...
                            >
                              {(hasFullDuplicate(house.duplicates) && ListingStatus.DUPLICATE) ||
                                (house.isActive ? ListingStatus.CALL : ListingStatus.NOT_ACTIVE)}
                            </Button>
                            <Button
                              sx={{
                                padding: '4px 10px',
                                visibility: `${hasFullDuplicate(house.duplicates) && 'hidden'}`,
                                borderRadius: '8px',
                                marginBottom: '8px',
                              }}
...
export default HouseFavoriteTable;
