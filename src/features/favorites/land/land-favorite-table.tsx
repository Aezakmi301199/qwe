...
const LandFavoriteTable: React.FC<FlatFavoriteTableProps> = observer(
  ({
    columnVisibility,
    ...
    }, [landStore.page, landStore.rowsPerPage]);
    ...
    socket
      ...
      .on(WsEvent.REAL_ESTATE_SAVED, async ({ realEstateId, userId }: { realEstateId: string; userId: string }) => {
        ...
        await landStore.removeFromFavourites(realEstateId);
      });
    socket
                ...
                {landStore.realEstate.map((land: Land) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {columnVisibility.area && (
                        <TableCell align='right' sx={{ verticalAlign: 'top', padding: '6px 0 0 16px' }}>
                          {land.area}
                        </TableCell>
                      )}
                      <TableCell align='right' sx={{ verticalAlign: 'top', padding: '4px 16px 0 0px' }}>
                        <Stack flexDirection={'row'} alignItems={'flex-start'} justifyContent={'end'}>
                          ...
                          <Box visibility={isPriceHistoryVisible(land.price, land.priceHistory) ? 'visible' : 'hidden'}>
                        ...
                        {land.callingUser ? (
                          ...
                          >
                            <Button
                              ...
                              disabled={
                                Boolean(land.callingUser) || hasFullDuplicate(land.duplicates) || !land.isActive
                              }
                              variant={'contained'}
                              color='primary'
                              size={Size.SMALL}
                              sx={{
                                ...
                                fontWeight: '400',
                                color:
                                  hasFullDuplicate(land.duplicates) || !land.isActive
                                    ? `${theme.palette.text.primary} !important`
                                    : 'white',

                                borderRadius: `${land.isActive && !hasFullDuplicate(land.duplicates) ? '8px' : '45px'}`,
                                textTransform: 'none',
                                whiteSpace: 'nowrap',
                                '&:hover': {
                                  background: '#2b63be',
                            ...
                            >
                              {(hasFullDuplicate(land.duplicates) && ListingStatus.DUPLICATE) ||
                                (land.isActive ? ListingStatus.CALL : ListingStatus.NOT_ACTIVE)}
                            </Button>
                            <Button
                              sx={{
                                padding: '4px 10px',
                                visibility: `${hasFullDuplicate(land.duplicates) && 'hidden'}`,
                                borderRadius: '8px',
                                marginBottom: '8px',
                              }}
...
export default LandFavoriteTable;
